...
exports.getAttendanceByUser = async (req, res) => {
  try {
    ...
    const result = await attendanceService.getAttendanceByUser(requestedUserId, req.query);
    return res.json(result);
  } catch (error) {
    return handleError(res, error);
  }
...
exports.exportAttendanceExcel = async (req, res) => {
  try {
    const fileData = await attendanceService.exportAttendanceWorkbookBuffer(req.query);
