...
const createTransporter = () => {
  ...
  try {
    ...
    const error = new Error("Nodemailer is not installed. Run npm install nodemailer in backend.");
  ...
  if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
    const error = new Error("Email service is not configured. Add SMTP_HOST, SMTP_PORT, SMTP_USER and SMTP_PASS.");
    error.statusCode = 500;
    throw error;
  }
...
exports.getMySalarySlips = async (req, res) => {
  try {
    const slips = await SalarySlip.find({ userId: req.user.id })
