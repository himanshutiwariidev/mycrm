...
#### Contracts Tab
- List of contracts linked to this client
- Create/edit contracts with: `contractNumber`, `projectName`, `projectDescription`, `projectScope`, `timeline`, `projectAmount`, `currency`, `paymentTerms`, `validUntil`, `contractStatus` (draft/sent/accepted/rejected/expired)
- Deliverables sub-list per contract
- Payment schedule sub-list per contract
...
- Entry types: `created`, `updated`, `status_changed`, `contract_added`, `payment_added`, `reminder_added`, `pi_uploaded`, `pi_deleted`, `user_assigned`, `remark_added`, `progress_updated`
...
| Contracts | Manage all contracts across all clients |
