...
const DeliverablesTracker = ({ contract, onUpdated }) => {
  ...
  const saveDelivered = async (deliverableId, delivered) => {
    ...
    try {
      const response = await updateDeliverable(contract._id, deliverableId, { delivered });
  ...
  const saveStatus = async (deliverableId, status) => {
    setSavingId(deliverableId);
    try {
      const response = await updateDeliverable(contract._id, deliverableId, { status });
      onUpdated(response.data.contract);
    } catch (err) {
  ...
  const handleAddDeliverable = async (e) => {
    ...
    try {
      const response = await addDeliverable(contract._id, {
  ...
  if (!deliverables.length) {
    return (
      <div className="deliverables-tracker">
        <div className="empty-state">No deliverables added for this contract yet.</div>
        {addDeliverableForm}
      </div>
    );
  }
  ...
  return (
    <div className="deliverables-tracker">
      {deliverables.map((item) => {
        const isQuantityMode = item.trackingMode !== "status";
        ...
        const leafConfig = item.categoryId && item.path ? getLeafConfig(item.categoryId, item.path) : null;
        const selection = leafConfig ? findMatchingSelection(contract, item.categoryId, item.path) : null;
          ...
          <div className="deliverable-row" key={item._id}>
            <div className="deliverable-header">
              ...
              {isQuantityMode ? (
                ...
                <select
                  className="deliverable-status-select"
                  value={status}
                  disabled={savingId === item._id}
                  onChange={(e) => saveStatus(item._id, e.target.value)}
            ...
            {isExpanded && <DeliverableDetailPanel leafConfig={leafConfig} selection={selection} currency={contract?.currency} />}
